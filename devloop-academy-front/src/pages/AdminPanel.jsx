...
export default function AdminPanel() {
  ...
  const fetchAdminData = async () => {
    ...
    const [coursesRes, usersRes, settingsRes] = await Promise.all([
      ...
      supabase.from('site_settings').select('payment_mode').eq('id', 'main').single()
    ]);
  ...
  const togglePaymentMode = async (newMode) => {
  ...
  return (
    <div className="max-w-6xl mx-auto py-10 px-4">
      ...
      <div className="bg-gray-900 p-6 rounded-2xl border border-gray-800 mb-8">
        ...
        <p className="text-gray-500 text-[10px] mt-3 uppercase tracking-widest font-bold">
          Estado actual: <span className={mode === 'prod' ? 'text-green-500' : 'text-yellow-500'}>{mode}</span>
        </p>
      </div>
        ...
        <div className="space-y-8">
          {!editingCourse ? (
            ...
            <div className="space-y-6">
              <button onClick={() => setEditingCourse(null)} className="text-gray-400 hover:text-white text-sm flex items-center gap-2 mb-4">
              ...
              <tbody className="divide-y divide-gray-800">
                {users.map(u => (
                  <tr key={u.id} className="hover:bg-gray-900/30 transition-colors">
                    ...
                    <td className="p-4">
                      {u.is_active ? <span className="text-green-500 text-[10px] font-bold bg-green-500/10 px-2 py-0.5 rounded border border-green-500/20 uppercase">Activa</span> : <span className="text-gray-500 text-[10px] font-bold bg-gray-500/10 px-2 py-0.5 rounded border border-gray-500/20 uppercase">Inactiva</span>}
                    </td>
                    <td className="p-4">
                      <select className={`text-xs font-bold px-2 py-1.5 rounded outline-none border transition-colors ${u.role === 'admin' ? 'bg-red-900/20 text-red-400 border-red-900/30' : u.role === 'teacher' ? 'bg-purple-900/20 text-purple-400 border-purple-900/30' : 'bg-gray-900 text-gray-300 border-gray-700'}`} value={u.role || 'student'} onChange={(e) => updateUserRole(u.id, e.target.value)}>
                    ...
                    <td className="p-4 text-right">
                      <button onClick={() => toggleUserStatus(u.id, u.is_active)} className={`text-xs px-4 py-1.5 rounded-lg font-bold transition-all ${u.is_active ? 'bg-red-900/20 text-red-400 border border-red-900/30 hover:bg-red-900/40' : 'bg-blue-900/20 text-blue-400 border border-blue-900/30 hover:bg-blue-900/40'}`}>
