...
export default function Home() {
  ...
  useEffect(() => {
    // Verificamos si la URL termina en #planes o si el hash lo contiene
    if (window.location.hash.includes('planes')) {
      const element = document.getElementById('planes');
      if (element) {
        // Un pequeño timeout asegura que el componente se haya renderizado 
        // completamente antes de intentar el scroll
        setTimeout(() => {
          element.scrollIntoView({ behavior: 'smooth' });
        }, 50);
      }
    }
  }, [courses]); // Se ejecuta cuando los cursos cargan para asegurar que la página tiene su altura real
  ...
  return (
    <div className="space-y-16 py-8">
            ...
            {filteredCourses.map((course) => (
              ...
              >
                <div className="absolute top-4 right-4 z-10">
                  <span className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full border ${course.is_free
      ...
      {/* SECCIÓN: PLANES DE SUSCRIPCIÓN CON EL ID CORRECTO */}
